    ...
    CASE NEW.payment_frequency
      WHEN 'weekly' THEN
        IF NEW.payment_week_day IS NOT NULL THEN
          ...
          payment_date_to_insert := next_payment_date + 
            ((NEW.payment_week_day - EXTRACT(DOW FROM next_payment_date) + 7) % 7) * 
            INTERVAL '1 day';
          next_payment_date := payment_date_to_insert + INTERVAL '7 days';
        END IF;
      ...
      WHEN 'bi-weekly' THEN
        IF NEW.payment_week_day IS NOT NULL THEN
          ...
          payment_date_to_insert := next_payment_date + 
            ((NEW.payment_week_day - EXTRACT(DOW FROM next_payment_date) + 7) % 7) * 
            INTERVAL '1 day';
          next_payment_date := payment_date_to_insert + INTERVAL '14 days';
        END IF;

      WHEN 'bi-monthly' THEN
        -- Payments on 15th and last day of month
        IF EXTRACT(DAY FROM next_payment_date) < 15 THEN
          payment_date_to_insert := DATE_TRUNC('month', next_payment_date) + INTERVAL '14 days';
          next_payment_date := DATE_TRUNC('month', next_payment_date) + INTERVAL '1 month' - INTERVAL '1 day';
        ELSE
          payment_date_to_insert := DATE_TRUNC('month', next_payment_date) + INTERVAL '1 month' - INTERVAL '1 day';
      ...
      WHEN 'monthly' THEN
        IF NEW.payment_day IS NOT NULL THEN
          ...
          payment_date_to_insert := DATE_TRUNC('month', next_payment_date) + (NEW.payment_day - 1) * INTERVAL '1 day';
          -- If we're past this month's date, move to next month
          IF payment_date_to_insert < next_payment_date THEN
            payment_date_to_insert := payment_date_to_insert + INTERVAL '1 month';
          END IF;
          next_payment_date := DATE_TRUNC('month', payment_date_to_insert) + INTERVAL '1 month';
        END IF;
    END CASE;

    -- Insert payment if date was calculated
    IF payment_date_to_insert IS NOT NULL AND payment_date_to_insert <= end_date THEN
